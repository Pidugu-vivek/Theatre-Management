...
async function fillLatestandUpcoming(datevalue,city, latestmovies1, upcomingmovies1) {
  ...
  for (let i = 0; i <value1.length; i++) {
    ...
    if (value2 != null) {
      moviearrayBasedoncity = value2.moviestatusinfo;


      for (let j = 0; j < moviearrayBasedoncity.length; j++) {
        let value3 = await movieinfo.findOne({
          MovieId: moviearrayBasedoncity[j].MovieId,
        });
        //  console.log(value3);
        if (checkrepeatedmovies.includes(moviearrayBasedoncity[j].MovieId)) {
          continue;
        } else {
          checkrepeatedmovies.push(moviearrayBasedoncity[j].MovieId);
        }

        const date3 = new Date(convertdate(moviearrayBasedoncity[j].rentaldate));
        let rentaldate = date3.toISOString().slice(0, 10);
        let rentaldays=moviearrayBasedoncity[j].rentaldays;
        ...
        if (rentaldate.localeCompare(today) <= 0) {
          ...
          if (moviearrayBasedoncity[j].status == "Active") {
